...
class ForgeRenderContext : RenderContext {
    ...
    override fun drawColoredRect(left: Float, top: Float, right: Float, bottom: Float, color: Int) {
        RenderUtils.drawGradientRect(0, left.toInt(), top.toInt(), right.toInt(), bottom.toInt(), color, color)
    }
    ...
    override fun drawGradientRect(
        ...
        RenderUtils.drawGradientRect(zLevel, left, top, right, bottom, startColor, endColor)
    }
    ...
    override fun refreshScissor() {
        GlScissorStack.refresh(ScaledResolution(Minecraft.getMinecraft()))
    }
